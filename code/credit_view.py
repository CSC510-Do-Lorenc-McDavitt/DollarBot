...
def display_credit(chat_id):
    ...
    for account in credit_list[str(chat_id)]:
        ...
        if len(credit_list[str(chat_id)][account]["expenses"]) == 0:
            output.append("    " + "None")
        output.append("  Monthly Due Date: " +
                      str(credit_list[str(chat_id)][account]["due date"]))
        output.append("  Currently Owing: $" +
                      str("{:.2f}".format(credit_list[str(chat_id)][account]["owe"])))
